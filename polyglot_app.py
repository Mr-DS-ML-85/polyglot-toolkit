...
class PolyglotApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        items = [("◈","Dashboard",0),("◆","Builder",1),("⚠","Scanner",2),("▶","Monitor",3),
                 ...
                 ("💼","Workspace",16),("🌐","Net Tools",17),("⬡","Hex Editor",18),("🔵","Blue Side",19)]
        ...
        self.stack.addWidget(self._pg_dashboard())       # 0
        self.stack.addWidget(self._pg_builder())          # 1
        self.stack.addWidget(self._pg_scanner())           # 2
        self.stack.addWidget(self._pg_monitor())           # 3
        self.stack.addWidget(self._pg_training())          # 4
        self.stack.addWidget(self._pg_quarantine())        # 5
        self.stack.addWidget(self._pg_yara())              # 6
        self.stack.addWidget(self._pg_activity_log())      # 7
        self.stack.addWidget(self._pg_settings())          # 8
        self.stack.addWidget(self._pg_report())            # 9
        self.stack.addWidget(self._pg_recover_bak())       # 10
        self.stack.addWidget(self._pg_server())            # 11
        self.stack.addWidget(self._pg_deep_analysis())     # 12
        self.stack.addWidget(self._pg_monitoring_panel())  # 13
        self.stack.addWidget(self._pg_investigation())     # 14
        self.stack.addWidget(self._pg_benchmark())         # 15
        self.stack.addWidget(self._pg_workspace())         # 16
        self.stack.addWidget(self._pg_network_tools())     # 17
        self.stack.addWidget(self._pg_hex_editor())        # 18
        self.stack.addWidget(self._pg_blue_side())         # 19
    ...
    def _pg_monitoring_panel(self):
        p = QWidget(); l = QVBoxLayout(p); l.setContentsMargins(30,25,30,25); l.setSpacing(12)
        l.addWidget(self._header("📡 Monitoring Panel — 10 Views", T.CYAN))

        tabs = QTabWidget()
        tabs.setStyleSheet(f"QTabWidget::pane{{background:{T.BG2};border:1px solid {T.BORDER}}}QTabBar::tab{{background:{T.BG3};color:{T.DIM};padding:8px 16px;border:none}}QTabBar::tab:selected{{background:{T.BG2};color:{T.RED};border-bottom:2px solid {T.RED}}}")

        # Sub-panels
        tabs.addTab(self._mp_sub_tab("Live Logs","📜","Real-time log streaming from all engines"),"Live Logs")
        tabs.addTab(self._mp_sub_tab("Realtime Events","⚡","Event stream from scanner, monitor, and detector"),"Events")
        tabs.addTab(self._mp_sub_tab("Process Viewer","⚙","Running processes related to toolkit"),"Processes")
        tabs.addTab(self._mp_sub_tab("Connection Viewer","🔌","Network connections and listeners"),"Connections")
        tabs.addTab(self._mp_sub_tab("Alerts Panel","🔔","Critical alerts and notifications"),"Alerts")
        tabs.addTab(self._mp_sub_tab("File Change Monitor","📁","Filesystem change tracking"),"File Changes")
        tabs.addTab(self._mp_sub_tab("Terminal Activity","💻","Command execution log"),"Terminal")
        tabs.addTab(self._mp_sub_tab("Workspace Audit","📋","Workspace modification audit trail"),"Audit")
        tabs.addTab(self._mp_sub_tab("Session Replay","🔄","Replay past scanning sessions"),"Replay")
        tabs.addTab(self._mp_sub_tab("Dashboard Metrics","📊","Live metrics and counters"),"Metrics")

        l.addWidget(tabs, 1)
        return p

    def _mp_sub_tab(self, title, icon, description):
        """Create a monitoring sub-panel with log viewer and controls."""
        w = QWidget(); l = QVBoxLayout(w); l.setContentsMargins(15,10,15,10)
        l.addWidget(QLabel(f"{icon} {title}"))
        desc = QLabel(description); desc.setStyleSheet(f"color:{T.DIM};font-size:11px")
        l.addWidget(desc)
        log = log_box(); l.addWidget(log, 1)
        # Populate with initial info
        append_log(log, f"{title} initialized — {datetime.now().strftime('%H:%M:%S')}", T.DIM)
        append_log(log, "Waiting for events...", T.DIM)
        return w
    ...
    def _pg_investigation(self):
        ...
        tabs.addTab(self._inv_snapshots(),"Compare Snapshots")
    ...
    def _ws_regex(self):
        ...
        r = QHBoxLayout()
        self.regex_pattern = inp("Regular expression pattern...")
        r.addWidget(self.regex_pattern, 1)
        tb = btn("Test", T.BLUE); tb.clicked.connect(self._test_regex); r.addWidget(tb)
        l.addLayout(r)
        ...
        self.regex_input.setPlaceholderText("Paste text to match against...")
        l.addWidget(self.regex_input, 1)
        self.regex_results = log_box(); self.regex_results.setMaximumHeight(200)
        l.addWidget(self.regex_results)
        return w

    def _test_regex(self):
        import re
        pattern = self.regex_pattern.text()
        text = self.regex_input.toPlainText()
        self.regex_results.clear()
        try:
            matches = list(re.finditer(pattern, text))
            append_log(self.regex_results, f"Found {len(matches)} matches", T.GREEN)
            for m in matches[:50]:
                append_log(self.regex_results, f"  [{m.start()}:{m.end()}] \"{m.group()}\"", T.CYAN)
                if m.groups():
                    for i,g in enumerate(m.groups(),1):
                        append_log(self.regex_results, f"    Group {i}: \"{g}\"", T.DIM)
        except re.error as e:
            append_log(self.regex_results, f"Regex error: {e}", T.RED)
    ...
    def _pg_network_tools(self):
        ...
        tabs.addTab(self._nt_raw_request(),"Raw Request")
        tabs.addTab(self._nt_history(),"Request History")
    ...
    def _nt_raw_request(self):
        w = QWidget(); l = QVBoxLayout(w)
        l.addWidget(QLabel("📝 Raw HTTP Request Editor"))
        self.raw_req = QTextEdit()
        self.raw_req.setStyleSheet(f"background:{T.BG_IN};color:{T.FG};border:1px solid {T.BORDER};border-radius:6px;font-family:Consolas,monospace;font-size:12px")
        self.raw_req.setPlainText("GET / HTTP/1.1\r\nHost: example.com\r\nConnection: close\r\n\r\n")
        l.addWidget(self.raw_req, 1)
        rb = btn("Send Request", T.BLUE); rb.clicked.connect(self._send_raw_req); l.addWidget(rb)
        self.raw_resp = log_box(); l.addWidget(self.raw_resp, 1)
        return w
    ...
    def _pg_hex_editor(self):
        p = QWidget(); l = QVBoxLayout(p); l.setContentsMargins(30,25,30,25); l.setSpacing(12)
        l.addWidget(self._header("⬡ Hex Editor", T.GREEN))

        # File selector
        fr = QHBoxLayout()
        self.hex_file = inp("File to view...")
        fr.addWidget(self.hex_file, 1)
        bb = QPushButton("Browse"); bb.setCursor(Qt.CursorShape.PointingHandCursor)
        bb.setStyleSheet(f"background:{T.BLUE};color:white;border:none;border-radius:4px;padding:8px 16px")
        bb.clicked.connect(lambda: self._browse(self.hex_file))
        fr.addWidget(bb)
        lb = btn("Load", T.GREEN); lb.clicked.connect(self._load_hex); fr.addWidget(lb)
        l.addLayout(fr)

        # Search bar
        sr = QHBoxLayout()
        self.hex_search = inp("🔍 Hex pattern (e.g., 89504e47) or ASCII text...")
        sr.addWidget(self.hex_search, 1)
        sb = btn("Search", T.YELLOW); sb.clicked.connect(self._hex_search); sr.addWidget(sb)
        l.addLayout(sr)

        # Tabs for different views
        tabs = QTabWidget()
        tabs.setStyleSheet(f"QTabWidget::pane{{background:{T.BG2};border:1px solid {T.BORDER}}}QTabBar::tab{{background:{T.BG3};color:{T.DIM};padding:8px 16px;border:none}}QTabBar::tab:selected{{background:{T.BG2};color:{T.GREEN};border-bottom:2px solid {T.GREEN}}}")

        # Hex dump
        self.hex_dump = QTextEdit(); self.hex_dump.setReadOnly(True)
        self.hex_dump.setStyleSheet(f"background:{T.BG_IN};color:{T.FG};border:none;font-family:Consolas,monospace;font-size:12px")
        ...
        tabs.addTab(self.hex_format,"Format Detection")
    ...
    def _load_hex(self):
        ...
        self.hex_dump.clear()
        lines = []
        for i in range(0, min(len(data), 50000), 16):
            chunk = data[i:i+16]
        ...
        for i in range(0, min(len(data), 10000), block_size):
            ...
            ent = self.builder.entropy(block)
            bar = '█' * int(ent * 5)
            color = T.RED if ent > 7.5 else T.YELLOW if ent > 6.0 else T.GREEN
            append_log(self.hex_entropy, f"0x{i:08x}: {ent:.3f} {bar}", color)

        # Format detection
        self.hex_format.clear()
        signatures = [
            (b'\xff\xd8\xff', 'JPEG'), (b'\x89PNG', 'PNG'), (b'GIF8', 'GIF'),
            (b'%PDF', 'PDF'), (b'PK\x03\x04', 'ZIP'), (b'MZ', 'PE/DOS'),
            (b'\x7fELF', 'ELF'), (b'\xfe\xed\xfa', 'Mach-O'), (b'Rar!', 'RAR'),
            (b'\x1f\x8b', 'GZIP'), (b'BM', 'BMP'), (b'\x00\x00\x01\x00', 'ICO'),
            (b'RIFF', 'RIFF'), (b'OggS', 'OGG'), (b'\x1a\x45\xdf\xa3', 'MKV/WebM'),
            (b'ID3', 'MP3'), (b'\xff\xfb', 'MP3'), (b'fLaC', 'FLAC'),
            (b'\xd0\xcf\x11\xe0', 'OLE2'), (b'SQLite', 'SQLite'),
        ]
        for sig, name in signatures:
            if data[:len(sig)] == sig:
                append_log(self.hex_format, f"  ✓ {name} signature at offset 0x00", T.GREEN)

        # Scan for embedded signatures
        found = []
        for sig, name in signatures[1:]:  # skip first (already matched)
            pos = data.find(sig, 1)
            if pos > 0 and pos < len(data) - 1:
                found.append((pos, name))
        if found:
            append_log(self.hex_format, f"\n  ⚠ Embedded signatures found:", T.YELLOW)
            for pos, name in sorted(found):
                append_log(self.hex_format, f"    {name} at 0x{pos:08x}", T.ORANGE)
        else:
            append_log(self.hex_format, "\n  No embedded signatures detected", T.DIM)

    def _hex_search(self):
        if not hasattr(self, 'hex_data'): return
        query = self.hex_search.text().strip()
        if not query: return
        self.hex_dump.clear()
        ...
        try:
            ...
            lines = [f"Found {len(positions)} occurrences of hex pattern '{query}':\n"]
            for p in positions[:50]:
                chunk = self.hex_data[p:p+len(pattern)+16]
                hex_part = ' '.join(f'{b:02x}' for b in chunk)
                lines.append(f"  0x{p:08x}: {hex_part}")
            self.hex_dump.setPlainText('\n'.join(lines))
        except ValueError:
            # ASCII search
            ...
            while True:
                ...
                start = pos + 1
                if len(positions) > 100: break
            lines = [f"Found {len(positions)} occurrences of '{query}':\n"]
            for p in positions[:50]:
                lines.append(f"  0x{p:08x}")
            self.hex_dump.setPlainText('\n'.join(lines))
    ...
    def _pg_blue_side(self):
        p = QWidget(); l = QVBoxLayout(p); l.setContentsMargins(30,25,30,25); l.setSpacing(12)
        l.addWidget(self._header("🔵 Blue Side Monitoring — 9 Views", T.BLUE))

        tabs = QTabWidget()
        tabs.setStyleSheet(f"QTabWidget::pane{{background:{T.BG2};border:1px solid {T.BORDER}}}QTabBar::tab{{background:{T.BG3};color:{T.DIM};padding:8px 16px;border:none}}QTabBar::tab:selected{{background:{T.BG2};color:{T.BLUE};border-bottom:2px solid {T.BLUE}}}")

        tabs.addTab(self._bs_panel("Network Logs","📡","Incoming/outcoming connection logs"),"Net Logs")
        tabs.addTab(self._nt_history(),"Req History")
        tabs.addTab(self._bs_panel("WebSocket Monitor","🔌","Live WebSocket connection tracking"),"WebSocket")
        tabs.addTab(self._nt_dns(),"DNS")
        tabs.addTab(self._nt_whois(),"Whois")
        tabs.addTab(self._nt_tcp(),"TCP Test")
        tabs.addTab(self._bs_panel("Connection Viewer","🔗","Active connections and listeners"),"Connections")
        tabs.addTab(self._bs_panel("Process Viewer","⚙","Running process list"),"Processes")
        tabs.addTab(self._bs_panel("File Changes","📁","Filesystem modification alerts"),"File Changes")
